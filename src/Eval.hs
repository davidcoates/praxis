...
evalInbuilt inbuilt specialization = return $ case inbuilt of
  ...
  InbuiltMultiply -> liftIII (*)
  InbuiltNegate   -> liftI $ \(con, decon) -> Value.Fn (\x -> return (con (negate (decon x))))
  InbuiltGetInt   -> liftI $ \(con, _)     -> Value.Fn (\Value.Unit -> liftIOUnsafe (con <$> readLn))
  InbuiltPutInt   -> liftI $ \(_, decon)   -> Value.Fn (\i -> liftIOUnsafe (print (decon i) >> pure Value.Unit))
  InbuiltEq       -> liftIIB (==)
  InbuiltNeq      -> liftIIB (/=)
  InbuiltLt       -> liftIIB (<)
  InbuiltGt       -> liftIIB (>)
  InbuiltLte      -> liftIIB (<=)
  InbuiltGte      -> liftIIB (>=)
  InbuiltGetStr   -> Value.Fn (\Value.Unit -> Value.String <$> liftIOUnsafe getContents)
  InbuiltPutStr   -> Value.Fn (\(Value.String s) -> liftIOUnsafe (putStr s) >> pure Value.Unit)
  InbuiltPutStrLn -> Value.Fn (\(Value.String s) -> liftIOUnsafe (putStrLn s) >> pure Value.Unit)
  InbuiltCompose  -> Value.Fn (\(Value.Pair (Value.Fn f) (Value.Fn g)) -> pure (Value.Fn (\x -> g x >>= f)))
  InbuiltPrint    -> Value.Fn (\x -> liftIOUnsafe (print x) >> pure Value.Unit)
  InbuiltNewArray -> Value.Fn (\(Value.Pair (Value.USize n) v) -> Value.newArray n v)
  ...
  InbuiltDrop     -> Value.Fn (\_ -> pure Value.Unit) -- memory is managed by the Haskell runtime
