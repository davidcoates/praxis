...
view : forall ?v a b . ?v ( a , b ) -> ( ?v b , ?v a ) = \ ( [?v a] x , [?v b] y ) -> ( [?v b] y , [?v a] x )
|]
...
|]
    ...
    it "type checks" $ runPretty (check ProgramT program) `shouldReturn` trim [r|
datatype unboxed Box &v !a = [forall &v !a . &v !a -> Box &v !a] Box &v !a
rec
...
|]) `shouldReturn` "[( )] let [List I32] xs = [( I32 , List I32 ) -> List I32] Cons ( [I32] 1 , [( I32 , List I32 ) -> List I32] Cons ( [I32] 2 , [( I32 , List I32 ) -> List I32] Cons ( [I32] 3 , [( ) -> List I32] Nil [( )] ( ) ) ) ) in read xs in [( )] [&'l1 List I32 -> Box &'l1 ( List I32 )] Box [&'l1 List I32] xs seq [( )] ( )"
